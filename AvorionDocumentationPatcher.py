...
class AvorionDocumentationPatcher:
	...
	def run(self):
		...
		if html_dir == self.write_dir:
			raise AvorionDocumentationPatcherError("InputDirectory and OutputDirectory cannot be the same because reasons")
	...
	def _generateFile(self, doc_name):
		...
		# GENERATE BEGIN
		data = self._generateFunctionLinks(doc_name, data)
		# GENERATE END
	...
	def _generateFunctionLinks(self, doc_name, data):
		""" Generate patched 
			@param doc_name : str - the html file we are patching
		"""
		for name in self.html_map:
